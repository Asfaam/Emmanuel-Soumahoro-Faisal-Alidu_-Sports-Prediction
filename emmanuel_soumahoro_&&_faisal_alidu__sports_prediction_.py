# -*- coding: utf-8 -*-
"""Emmanuel Soumahoro && Faisal Alidu_ Sports Prediction .ipynb
...
## RandomForestRegressor has the least mean_square error hence we choose this model for deployment.
...
# Specifying that we want to exctract RandmForestRegressor
rfr = 'RandomForest'

model = models[rfr]

gs = GridSearchCV(model, params[rfr], cv=5)
gs.fit(x_train, y_train)
# Make predictions using the best model from GridSearchCV
y_pred = gs.predict(x_test)
...
#print('\nAccuracy: {}'.format(nb_model.score(x_test, y_test))
print(f"\nBest parameters for {rfr}: {gs.best_params_}")
print(f"Validation score for {rfr}: {gs.score(x_test, y_test)}")
#print()
print(f"mean_absolute_error for {rfr}: {mae}")
print(f"mean_squared_error for {rfr}: {mse}")
print(f"r2_score for {rfr}: {r2}\n")
...
"""### Saving RandomForestClassifier model using pickle"""
